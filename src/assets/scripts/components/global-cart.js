...
export default new Vue({
  ...
  mounted() {
    this.freeShipThreshold = Number(this.$el.dataset.freeShipThreshold);
  ...
  computed: {
    ...
    fromFreeShip() {
      let fromFreeShip = this.freeShipThreshold;
      if (Number(this.total) < this.freeShipThreshold) {
        fromFreeShip = (this.freeShipThreshold - this.total);
      } else {
        fromFreeShip = 0;
      }
  ...
  methods: {
    ...
    setUpAddEvent() {
      document.addEventListener('click', (event) => {
        if (event.target.classList.contains('atc-button')) {
          event.preventDefault();
          const { variantId, quantity } = event.target.dataset;
